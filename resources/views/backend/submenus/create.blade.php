    ...
    <div class="p-4 sm:ml-64 lg:ml-80">
        <div class="p-6 border-2 border-gray-200 border-dashed rounded-lg bg-white">
            <div class="flex justify-between items-center  mb-6">
                <h2 class="text-2xl font-semibold text-slate-900 mb-6">{{ __('Add Sub Menu') }}</h2>
            </div>
            <form action="{{ route('admin.submenus.store') }}" method="POST">
                ...
                <button type="submit"
                    class="bg-primary hover:bg-primary/80 text-white py-2 px-4 rounded-full">{{ __('Add Sub Menu') }}</button>
            </form>
        </div>
    </div>
@endsection
